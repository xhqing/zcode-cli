...
互動式啟動時，每 20 小時至多檢查一次 GitHub Release 最新版（按已安裝版本計），
有新版本時以非阻塞的更新卡片提示，附精確的更新指令與 release notes 連結。CI 環境
...
流程提供一條窄的 macOS 回呼橋。Node.js 啟動公開的指令，並作為擷取出的上游
...
Node.js 啟動器透過呼叫 runtime 公開的 `app-server` 協定補充市集操作；不 patch、不
